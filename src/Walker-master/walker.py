...
class Walker:
    ...
    def __init__(self, original_ppi, low_list, remove_nodes=None):
        if remove_nodes is None:
            remove_nodes = []
        self._build_matrices(original_ppi, low_list, remove_nodes)

    def run_exp(self, source, restart_prob, og_prob, node_list=None):
    ...
    def _generate_prob_list(self, p_t, node_list):
        gene_probs = dict(zip(self.node_list, p_t.tolist()))
    ...
    def _generate_rank_list(self, p_t):
        ...
        """
        gene_probs = zip(self.node_list, p_t.tolist())
    ...
    def _set_up_p0(self, source):
        ...
        for source_id in source:
            try:
                ...
                source_index = self.node_to_index[source_id]
    ...
    def _build_matrices(self, original_ppi, low_list, remove_nodes):
        ...
        if remove_nodes:
            ...
            largest_component = max(nx.connected_components(original_graph),
                                    key=len)
            original_graph = original_graph.subgraph(largest_component).copy()

        self.OG = original_graph
        self.node_list = list(original_graph.nodes())
        self.node_to_index = {
            node: index for index, node in enumerate(self.node_list)
        }

        og_not_normalized = nx.to_numpy_array(
            original_graph, nodelist=self.node_list, dtype=float
        )
    ...
    def _tsg_matrix(self, original_graph, og_matrix, low_list):
        ...
        for line in list_fp.readlines():
            ...
            if len(split_line) < 2:
                continue
            node = split_line[0]
            if split_line[1] == 'NA' and node in self.node_to_index:
                index_list.append(self.node_to_index[node])
